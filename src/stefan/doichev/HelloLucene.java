...
public class HelloLucene {
	public static void main(String[] args) throws IOException, ParseException {
		...
		DecimalFormat df = new DecimalFormat("0.000"); 	//used to round the TF_IDF in the result.
		...
		for(int i=0;i<hits.length;++i) {
			...
			if (terms != null && terms.size() > 0) {
				...
				while ((term = termsEnum.next()) != null) {// explore the terms for this field					
					DocsEnum docsEnum = termsEnum.docs(null, null); // enumerate through documents
					while (docsEnum.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
						if (term.utf8ToString().equalsIgnoreCase(querystr)) {
							TF = docsEnum.freq(); //get the term frequency in the document
							TF_IDF = TF*IDF;
							System.out.println("TF-IDF(Term:" + querystr + ", Doc:" + d.get("title") + ") = " + df.format(TF_IDF));
						}
					}
				}
			}
